% =====================================================================
%  primat — extensive user + physics documentation
%
%  Figures are produced by the companion notebook in this folder,
%      manual/primat_doc_figures.ipynb
%  which writes PDF figures into manual/figures/.  Re-run that notebook
%  (Kernel -> Restart & Run All) before recompiling this document if the
%  code has changed.
%
%  Compile with:   latexmk -pdf primat_documentation_v0.3.2.tex
%             or:  pdflatex primat_documentation_v0.3.2.tex  (x3 for refs)
% =====================================================================
...
\title{\textbf{primat}\\[2pt]
       \large A Python package (with a compiled C engine) for precision Big
       Bang Nucleosynthesis\\[6pt]
       \normalsize User guide and physics reference\\[4pt]
       \normalsize\textit{Documentation for primat version 0.3.2}}
\author{Cyril Pitrou}
\date{\today}
...
\begin{abstract}
\noindent
\code{primat} (formerly \code{PyPRIMAT}) is a re-implementation of the
\code{PRIMAT} code for precise Big Bang Nucleosynthesis (BBN), distributed as
a single pip-installable package with two interchangeable backends: a fast
compiled C engine (the default) and a pure-Python implementation (used as a
fallback and for development/extensibility, e.g.\ custom reaction networks
via the GUI). Either backend integrates the coupled equations governing
...
\code{primat\_doc\_figures.ipynb}. This document describes \textbf{primat
version 0.3.2}: the package was renamed from \code{pyprimat} to \code{primat}
and gained the compiled C backend, a Streamlit GUI with a custom-network
builder, Monte-Carlo rate-uncertainty propagation, and a data-directory
overlay mechanism since the v0.1.0 edition of this document; see
Sec.~\ref{sec:usage} for all of these.
\end{abstract}
...
\code{primat} can be driven in four equivalent ways -- a command-line tool, a
Python API, a graphical interface, or (behind all three) the low-level
backend dispatcher -- all of which build the same parameter dictionary and
ultimately run the same physics. Run from the repository root so that the
data files under \code{primat/data/} resolve, or use a \code{pip install}ed
copy (data ships inside the package, see the ``Rates directory resolution''
paragraph below). Every code example below uses the shipped \emph{compiled C
engine}, the default and fastest of the two interchangeable backends
described in \S\ref{sec:backends}.

\subsection{Command-line interface}
The \code{primat} console script exposes the most commonly varied options:
...
D/H        = 2.4356594e-05
He3/H      = 1.0396011e-05
He3/He4    = 1.2675916e-04
Li7/H      = 5.503013e-10
Li6/Li7    = 1.418195e-05
--- running time: 0.69 seconds ---
\end{lstlisting}
The main named flags are \code{-{}-Omegabh2}, \code{-{}-DeltaNeff},
\code{-{}-network} (\code{small}/\code{small\_parthenope}/\code{large} or any
custom network file under \code{data/nuclear/networks/}), \code{-{}-amax}
(restrict any network to nuclides of mass number $\le A$; e.g.\ the example
above reproduces the old ``medium'' network's 68 reactions, \S\ref{sec:eras}),
\code{-{}-numerical\_precision}, \code{-{}-backend}
\{\code{auto},\code{c},\code{python}\} (\S\ref{sec:backends}),
\code{-{}-mc N} (also run an $N$-sample Monte-Carlo rate/$\tau_n$-uncertainty
propagation and print each observable as ``value +/- sigma'',
\S\ref{sec:mc-usage}), \code{-{}-json} (dump the full result dictionary
instead of the summary), \code{-{}-verbose}, \code{-{}-cache-info}/
\code{-{}-cache-clear} (inspect/clear the cached weak-rate files), and
\code{-{}-credits}/\code{-{}-version}. Any other \code{PRIMATConfig} key,
including a per-reaction \code{p\_<reaction>}/\code{delta\_<reaction>} rate
variation (\S\ref{sec:nuclear}), can be set with the generic escape hatch
\code{-{}-set KEY=VALUE} (repeatable), e.g.\
\code{-{}-set T\_end\_MeV=1e-4 -{}-set decay\_era=True}.

\subsection{Python API}
For full control, build a parameter dictionary and call \code{PRIMAT}
directly; every key of \code{config.DEFAULT\_PARAMS} (95 keys) is accepted.
\begin{lstlisting}[style=py]
from primat import PRIMAT

result = PRIMAT({"Omegabh2": 0.022425, "network": "large", "amax": 8}).solve()
print(f"YP  (BBN) = {result['YPBBN']:.8f}")   # 0.24700444
print(f"D/H       = {result['DoH']:.7e}")      # 2.4356398e-05
print(f"Neff      = {result['Neff']:.8f}")     # 3.04397730
\end{lstlisting}
\code{solve()} (equivalently \code{primat\_results()}, kept for backwards
compatibility) returns a dictionary with \code{Neff}, the helium mass
fractions \code{YPBBN}/\code{YPCMB}, the ratios \code{He4oH}, \code{DoH}, \code{He3oH},
\code{He3oHe4}, \code{Li7oH} (and, for networks tracking them, \code{Li6oLi7}
and CNO yields), and the neutrino observables
\code{Omeganurel}/\code{OneOverOmeganunr}. \code{PRIMAT} is a thin facade over
the background and nuclear-network classes: with the \emph{Python} backend
(\code{force\_backend="python"}, or automatically whenever a Python-only
feature such as \code{extra\_rho}/\code{background=}/\code{decay\_era} is
used) the instance also exposes the background history and abundance
trajectories as attributes used throughout the companion notebook: the
temperature vectors \code{\_Tg\_vec}, \code{\_Tnue\_vec}, the weak-rate
interpolants \code{\_nTOp\_frwrd}, \code{\_nTOp\_bkwrd} (functions of $\Tg$ in
\emph{kelvin}), the compiled forward reaction rates
\code{nucl.<reaction>\_frwrd} (function of $T$ in kelvin, internally
$T_9=T/10^9$), the abundance interpolant \code{\_Y\_of\_t}, and the final
values \code{\_Y\_final}. The C backend does not expose these
intermediate attributes (only the final result dict and, optionally, the
time-evolution arrays below), which is why the figures in this document are
generated with the Python backend even though it is no longer the default
for routine runs.

\paragraph{The \code{primat.backend} dispatcher.}\label{sec:backends}
\code{PRIMAT(...)}/\code{.solve()} always uses the pure-Python engine;
day-to-day scripts should instead call the dispatcher directly,
\begin{lstlisting}[style=py]
from primat.backend import run_bbn
result = run_bbn(params={"Omegabh2": 0.022425}, force_backend="auto")
\end{lstlisting}
which picks the compiled C extension (\code{primat.\_primat\_c}, wrapping the
standalone \code{primat-c/} C99 port) when it built successfully
(\code{primat.backend.HAS\_C\_BACKEND}), falls back silently to Python
otherwise, and returns the identical result-dict shape either way (the two
backends are held to a relative $5\times10^{-5}$ budget on $\DoH$ for the same
inputs, against a measured gap of $6.6\times10^{-6}$; that residual is each
backend's own ODE-tolerance error, and $\Neff$ agrees to every digit --- see
\code{tests/test\_backend\_parity.py}). Pass
\code{force\_backend="c"} to require the compiled engine (raises if it is
unavailable or if a Python-only feature is requested) or
\code{force\_backend="python"} to force the reference implementation. Custom
reaction networks (\S\ref{sec:custom-networks}), the full time-evolution
schema (below), \code{extra\_rho} and \code{decay\_era} are supported
identically on both backends, and incremental MC sample reuse
(\code{prev=}, \S\ref{sec:mc-usage}) works within either backend --- but not
across the two, whose random streams differ. Only \code{background=} (a custom
\code{Background} object, which cannot cross the C ABI) is Python-only.

\paragraph{Monte-Carlo rate-uncertainty propagation.}\label{sec:mc-usage}
Every tabulated reaction rate and $\tau_n$ carries a $1\sigma$ uncertainty
(\S\ref{sec:nuclear}); \code{primat.backend.run\_mc} (or the CLI's
\code{-{}-mc}) draws $N$ independent samples -- each reaction's
\code{p\_<reaction>}$\sim\mathcal N(0,1)$ and $\tau_n\sim\mathcal
N(\tau_n,\sigma_{\tau_n})$ -- re-solves BBN for each, and returns an
\code{MCResult} with the central (unperturbed) value, sample mean/std, and
every individual sample for each requested observable:
\begin{lstlisting}[style=py]
from primat.backend import run_mc
mc = run_mc(200, params={"network": "large", "amax": 8}, seed=0)
print(f"D/H = {mc['DoH'].central:.7e} +/- {mc['DoH'].std:.2e}")
\end{lstlisting}
The C backend runs one pthread per sample with an independent
xoshiro256** stream (not bit-for-bit comparable to the Python/NumPy path,
only statistically); both cap each reaction's sampled rescaling factor to
\code{mc\_rate\_rescale\_cap} (default 30, \S\ref{sec:nuclear}) to keep a
handful of poorly-constrained placeholder-uncertainty rates from dominating
the variance with unphysical outliers.

\paragraph{Custom reaction networks.}\label{sec:custom-networks}
Both backends accept a \code{custom\_network} override (remove/replace/add
reactions and swap in an alternate rate table for any reaction), built either
programmatically (\code{primat.network\_data.UpdateNuclearRates}) or through
the GUI's ``Import/Create custom network'' popups below, which can export the
result as a re-importable \code{.zip}.

\paragraph{Output files and the unified time-evolution schema.}
\code{output\_time\_evolution=True} populates (in memory, on both backends)
an \code{EvolutionResult} with the unified schema documented in
\code{primat/evolution.py}'s module docstring -- \code{t\_s}, \code{a},
\code{T\_gamma\_MeV}, the three flavour neutrino temperatures, and every
tracked nuclide's \code{Y\_<nuclide>} column, one row per output time step --
and additionally writes it to \code{output\_file} (default
\code{results/output\_tables.tsv}) if that is set (\code{output\_file=None}
skips the disk write, e.g.\ for the GUI, while still populating the
in-memory result). \code{output\_final\_result=True} writes the final mass
fractions to \code{output\_final\_file}; \code{output\_background\_evolution}/
\code{output\_mc\_samples} similarly control the background-only and
MC-samples TSV dumps.

\paragraph{Rates directory resolution.}
Two \code{PRIMATConfig} fields let a run substitute alternate data without
touching the installed package: \code{data\_dir} (a full replacement of the
shipped \code{primat/data/} tree -- must contain \code{NEVO/},
\code{nuclear/}, \code{csv/} and
\code{cache\_plasma\_weak/\{weak,plasma\}/}) and \code{user\_nuclear\_dir}
(an \emph{additive} overlay for nuclear networks and rate tables only,
checked before the shipped tree so built-in networks like \code{small}
remain available). Both are validated as existing directories at
\code{PRIMATConfig} construction time and are ordinary string parameters on
the C side too (\code{-{}-data\_dir}/\code{CPRIMAT\_DATA\_DIR} for the
standalone C CLI).

\subsection{Graphical interface}
After \code{pip install ".[gui]"}, a browser-based Streamlit app is available:
\begin{lstlisting}[style=sh]
primat-gui                          # console-script launcher
primat-gui --backend python         # force the Python backend (testing)
streamlit run primat/gui/app.py     # from a source checkout
\end{lstlisting}
It exposes the parameters as form fields, shows the final abundances and an
interactive abundance-evolution plot, and additionally offers, in the
``Nuclear reactions'' sidebar group, a single \textbf{Manage networks} button
-- the one gateway to listing, selecting, removing or renaming a network built
this session, loading one from a \code{.zip}, or opening \textbf{Create new
network}, which lets a user start from any named network, toggle reactions
in/out grouped by mass-number category, substitute or upload an alternate rate
table per reaction, override a decay rate and add brand-new reactions, then
save the result as a re-importable \code{.zip} or run BBN with it
(\S\ref{sec:custom-networks}); a results tab exposes the
same Monte-Carlo uncertainty propagation as \code{-{}-mc}
(\S\ref{sec:mc-usage}).
...
$\DoH \simeq 2.4350\times10^{-5}$ and $\Neff \simeq 3.04398$.
...
electromagnetic-plasma entropy $s\,a^3$ (\PR\ Eqs.~21--24).
...
once at start-up (\code{PRIMAT.\_setup\_background\_and\_cosmo}).
...
\code{external\_scale\_factor=True}, $a(T)$ is instead read directly from the
NEVO table's column $x\propto a$ (requires \code{incomplete\_decoupling=True};
the two modes agree to $\sim10^{-5}$ in $a(T)/t(T)$).
...
through the \code{extra\_rho} hook of \code{PRIMAT} (a list of functions
$\rho(\Tg)\,[\mathrm{MeV}^4]$; Python backend only, \S\ref{sec:backends}). Two
are built in: a constant extra radiation density from $\Delta\Neff$ decoupled
species (\code{plasma.rho\_nu\_extra},
$\rho=\tfrac78\,\tfrac{\pi^2}{15}\Tnu^4\,\Delta\Neff$), and Early Dark Energy
(\code{fEDE}, \code{zcEDE}, \code{wnEDE}; \code{PRIMAT.\_setup\_EDE}) -- a fluid
...
entropy, raising $\Neff$ above 3. primat reads the precomputed
...
the QED corrections are loaded from \code{data/cache\_plasma\_weak/plasma/QED\_*.txt} or computed
analytically by \code{qed\_pressure.py}; the Friedmann rate is assembled in
\code{PRIMAT.\_Hubble}. Figure~\ref{fig:decoupling} shows the per-flavour
...
recompute\_qed\_corrections & False & Recompute and overwrite \code{data/cache\_plasma\_weak/plasma/QED\_*.txt}. \\
external\_scale\_factor & False & Read $a(\Tg)$ directly from the NEVO table instead of solving the ODE. \\
DeltaNeff              & 0     & Extra decoupled radiation added to $\rho_{\rm tot}$. \\
munuOverTnu            & 0     & Neutrino degeneracy $\xi_\nu=\mu_\nu/\Tnu$. \\
data\_dir              & None  & Full-takeover replacement for the shipped \code{primat/data/} tree. \\
user\_nuclear\_dir      & None  & Additive overlay for nuclear networks \& rate tables only. \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{figure}[ht]
  ...
  \caption{Neutron mass fraction $X_n$ from primat (solid) versus the
...
\code{weak\_rates/} evaluates Eq.~\eqref{eq:born} and all corrections on the
background grid. Results are cached in \code{data/cache\_plasma\_weak/weak/} behind a configuration
...
tau\_n\_normalization   & True  & Normalise via $\tau_n$ (else from $G_F$, $V_{ud}$, $g_A$). \\
radiative\_corrections  & True  & $T=0$ Coulomb + resummed radiative corrections (CCR); \code{False} = bare Born rate. \\
finite\_mass\_corrections & True & Fokker--Planck finite-nucleon-mass correction. \\
thermal\_corrections    & True  & Finite-temperature radiative correction (CCRTh; Brown \& Sawyer 2001). \\
spectral\_distortions   & True  & Use the actual NEVO neutrino spectrum in the rates. \\
sampling\_nTOp\_per\_decade & 80 & Points per decade of $T$ in the $n\leftrightarrow p$ rate grid. \\
weak\_rate\_cache       & True  & Reuse the fingerprinted \code{data/cache\_plasma\_weak/weak/} cache when valid. \\
\bottomrule
\end{tabular}
\end{center}
...
dependence except for resonances. primat does not perform this average at run
...
\begin{itemize}\itemsep2pt
  ...
  \item \textbf{MT era} ($T_{\rm weak}\!\to\!T_{\rm nucl}$): the fixed
    18-reaction subset (even for \code{large} -- the full net is too stiff before
  ...
  \item \textbf{LT era} ($T_{\rm nucl}\!\to\!T_{\rm end}$): the full chosen
    network (12 / 68 (\code{large}, \code{amax=8}) / $\sim$429 reactions),
    seeded from the MT result, BDF + Jacobian.
\end{itemize}
...
$\text{NormWR}=1/(\tau_n F_n)$ in the \code{tau\_n\_normalization} mode ($F_n$
the decay phase-space factor). The reverse nuclear rates of Eq.~\eqref{eq:detbal}
are clamped at low $T_9$ so that the exothermic factor $e^{\gamma/T_9}$ cannot
overflow (the ``exothermic blow-up'' guard in \code{network\_data.py}).
...
differences (costly and noisy for $\sim$60 species), primat supplies it
...
\paragraph{$\YP^{\rm BBN}$ versus $\YP^{\rm CMB}$.}
$\YP^{\rm BBN}=4Y_{^4{\rm He}}$ is the helium \emph{nucleon} fraction: $Y_{^4{\rm He}}=
n_{^4{\rm He}}/n_b$ is the number of $^4$He per baryon, and multiplying by $A=4$
gives the fraction of \emph{baryons} (counted as nucleons) locked in $^4$He.
This is the natural BBN output but is not exactly a mass fraction, because a
bound $^4$He nucleus is lighter than four free nucleons (binding energy) and the
$^1$H atom mass differs slightly from one atomic mass unit. CMB likelihoods
instead parametrise helium by its \emph{true mass fraction}
$Y_P=\rho_{^4{\rm He}}/(\rho_{^4{\rm He}}+\rho_{^1{\rm H}})$, so for consistency
primat also returns
\begin{equation}
  \YP^{\rm CMB}=\frac{\tfrac14\,r_{\rm He}\,\YP^{\rm BBN}}
                    {\tfrac14\,r_{\rm He}\,\YP^{\rm BBN}+r_{\rm H}\,(1-\YP^{\rm BBN})},
  \qquad r_{\rm He}\equiv\frac{m_{^4{\rm He}}}{m_u},\ \ r_{\rm H}\equiv\frac{m_{^1{\rm H}}}{m_u},
\end{equation}
which reweights the nucleon fraction by the actual $^4$He and $^1$H masses (in
atomic mass units $m_u$); numerically $\YP^{\rm CMB}$ is about $0.5\%$ below
$\YP^{\rm BBN}$. Finally \code{Omeganurel} / \code{OneOverOmeganunr} are the
relativistic relic-neutrino $\Omega_\nu h^2$ and the inverse non-relativistic
$\Omega_\nu h^2$ per summed eV, built from the $T_\nu/\Tg$ ratio at $T_{\rm end}$
scaled to today through $T_0^{\rm CMB}$.

\subsection{Networks, eras, and rate variation}
The reaction list is named in
\code{data/nuclear/networks/\{small,small\_parthenope,large\}.txt} (or any
other network file of the same form, resolved through the
\code{user\_nuclear\_dir} overlay of \S\ref{sec:usage} if set);
\code{load\_network} parses it and \code{compile\_network} turns the
stoichiometry of Eq.~\eqref{eq:network} into fast right-hand-side and
Jacobian kernels (the three eras of Sec.~\ref{sec:eras} share these kernels).
Setting \code{amax} filters \emph{any} network (not just \code{large}) to
nuclides of mass number $\le$\code{amax}, e.g.\ \code{network="large",
amax=8} reproduces the former ``medium'' network's 68 reactions exactly
(App.~\ref{app:reactions}). Each reaction \code{<name>} carries a variation
knob \code{p\_<name>} sampling its rate at $\text{median}\times e^{p\,\sigma}$
for Monte-Carlo error propagation (\S\ref{sec:mc-usage}); the additive
\code{delta\_<name>} multiplies it by $1+\delta$ instead, and the combined
factor is clamped to \code{mc\_rate\_rescale\_cap} (default 30, both directions):
\begin{lstlisting}[style=py]
PRIMAT({"network": "large", "amax": 8,
      "p_d_p__He3_g": 1.0}).solve()   # d(p,gamma)He3 at +1 sigma
\end{lstlisting}
...
primat the full budget is obtained by Monte-Carlo (\S\ref{sec:mc-usage}),
drawing each \code{p\_<rxn>} from $\mathcal N(0,1)$ (and $\tau_n$ from its own
$1\sigma$); the Python-backend helper \code{mc\_uncertainty} parallelises this
with \code{joblib}, while \code{primat.backend.run\_mc}/\code{-{}-mc} dispatch
to the multi-threaded C implementation by default. Section~\ref{sec:sensitivity}
quantifies the individual responses.
...
\normalfont Option & Default & Effect \\
\midrule
network                 & "small" & Reaction set: \code{small}/\code{small\_parthenope} (12), \code{large} ($\sim$429), or any custom network file. \\
amax                    & None    & Drop reactions with any nuclide of mass number $> A$, for \emph{any} network. \\
...
rate\_grid\_npts        & 1000    & Points in the master $T_9$ grid for rate resampling. \\
p\_<reaction>           & 0       & Log-normal variation of an individual rate ($\times e^{p\sigma}$, MC). \\
delta\_<reaction>       & 0       & Additive fractional variation of an individual rate ($\times(1+\delta)$). \\
mc\_rate\_rescale\_cap  & 30      & Clamp on the combined $p$/$\delta$ rate-rescaling factor during MC. \\
output\_time\_evolution & False   & Populate (and, if \code{output\_file} is set, write) the full time series. \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{equation}
\begin{aligned}
  \frac{\delta m_e^2(p,T)}{T^2} &= \frac{4\afs}{\pi}\left[I^{(0,1)}_- + I^{(0,1)}_+(x)\right]
     - \frac{2\bar m_e^2\afs}{\pi\bar p}\!\int_0^\infty\!\dd\bar q\,
       \frac{\bar q}{\mathcal{E}_{\bar q}}
       \ln\!\left|\frac{\bar p+\bar q}{\bar p-\bar q}\right|\,
...
electromagnetic coupling $e$ (with $\afs=e^2/4\pi$). primat
(\code{qed\_pressure.py}) tabulates and \emph{sums the first two}, stored as the
two columns of \code{data/cache\_plasma\_weak/plasma/QED\_*.txt}:
\begin{align}
  \frac{\delta P_a}{T^4} &= \frac{\afs}{\pi}\!\left[
     -\frac{2}{3}I^{(0,1)}_+(x) - \frac{2}{\pi^2}\left(I^{(0,1)}_+(x)\right)^2\right]
...
\begin{equation}
  \Gamma^{\mathrm{RC}0}_{n\to p}=\Kw\!\int_0^\infty\! p^2\,\dd p\,
    \left[\mathcal F_+(E)R(E,E_\nu)\chi_+(E)
        +\mathcal F_+(-E)R(E,\Gap+E)\chi_+(-E)\right],
\end{equation}
...
\begin{align}
  ...
  g(E,k_{\max}) &= 3\ln\frac{m_p}{m_e}-\frac34
     + \frac{4}{\beta}L\!\left(\frac{2\beta}{1+\beta}\right)
     + 4[R(\beta)-1]\!\left[\frac{k_{\max}}{3E}-\frac32+\ln\frac{2k_{\max}}{m_e}\right]
     \nonumber\\
     &\quad + R(\beta)\left[2(1+\beta^2)+\frac{k_{\max}^2}{6E^2}-4\beta R(\beta)\right],
\end{align}
...
\begin{equation}
\begin{aligned}
  R(E,k_{\max}) = {}&\left[1+\tfrac{\afs}{2\pi}\left(g(E,k_{\max})-3\ln\tfrac{m_p}{2\Gap}\right)\right]
    \left(L+\tfrac{\afs}{\pi}C+\tfrac{\afs}{2\pi}\delta\right)\\
    &\times\left[S+\tfrac{\afs(m_p)}{2\pi}\left(\ln\tfrac{m_p}{m_A}+A_g\right)+\text{NLL}\right],
\end{aligned}
\end{equation}
where $L\simeq1.02094$ and $S\simeq1.02248$ are the (resummed) short- and
long-distance electroweak factors, $\afs(m_p)\simeq1/134$ is the fine-structure
constant run to the proton mass, and the two remaining constants are small
fixed higher-order pieces: $\delta$ is a two-loop/short-distance correction with
$\tfrac{\afs}{2\pi}\delta\simeq-4.3\times10^{-4}$, and $\mathrm{NLL}\simeq-10^{-4}$
is the next-to-leading-log remainder (\PR\ Eq.~A81). The reverse rate
...
\begin{aligned}
  \Gamma^{\gamma,T}_{n\to p}=\frac{\afs\Kw}{2\pi}\!\int_0^\infty\!\frac{\dd k}{k}
    \!\int_{m_e}^\infty\!\dd E\,
    \Big\{&A(E,k)\left[g(-E)\tilde\chi^A_+(E,k)+g(E)\tilde\chi^A_+(-E,k)\right]\\
    -\;&k\,B(E,k)\left[g(-E)\tilde\chi^B_+(E,k)+g(E)\tilde\chi^B_+(-E,k)\right]\Big\},
\end{aligned}
\end{equation}
with the stimulated-emission/absorption kernels
$\tilde\chi^A_\pm(E,k)=\tilde\chi_\pm(E-k)+\tilde\chi_\pm(E+k)-2\tilde\chi_\pm(E)$,
$\tilde\chi^B_\pm(E,k)=\pm\left[\tilde\chi_\pm(E-k)-\tilde\chi_\pm(E+k)\right]$
(with $\tilde\chi_\pm$ the Born kernel without its $p^2$ measure) and $A,B$ the
Bose factors of the photon bath. The electron-energy-shift part is
\begin{equation}
  \Gamma^{\Delta E,T}_{n\to p}=-\frac{2\afs\Kw}{\pi}\!\int\!\dd p\,
    \left[\chi_+(E)+\chi_+(-E)\right]
    \left[\frac{\pi^2T^2}{6}+\!\int_{m_e}^\infty\!\dd E'\,p'g(E')
    \left(1-\frac{m_e^2}{E'^2-E^2}\right)\right],
\end{equation}
with $E'=\sqrt{p'^2+m_e^2}$, and the proton--electron + electron self-energy
part is (\PR\ Eq.~106)
\begin{equation}
\begin{aligned}
  \Gamma^{ep+ee,T}_{n\to p}=\frac{\afs\Kw}{2\pi}\!\int_{m_e}^\infty\!\!\dd E\,
    &\left[\chi_+(E)+\chi_+(-E)\right]\!
    \int_{m_e}^\infty\!\!\dd E'\,g(E')\frac{E}{E'^2-E^2}\\
    \times\Big\{&8pp'-(E'^2+E^2)\ln\!\left(\frac{p+p'}{p-p'}\right)^{\!2}\\
    &-\frac{E'}{E}(E'^2+E^2)\ln\!\left[\frac{(EE'+pp')^2-m_e^4}{(EE'-pp')^2-m_e^4}\right]\Big\},
\end{aligned}
\end{equation}
all three with $\Gamma_{p\to n}=\Gamma_{n\to p}|_{\chi_+\to\chi_-}$. These
integrals are evaluated with the \code{vegas} Monte-Carlo library (or
...
$\epsilon\equiv T/m_N\sim10^{-3}$ near freeze-out. primat keeps them to first
...
of incomplete decoupling (\code{spectral\_distortions=True}). The distortion is
small, so the rate is \emph{linearized} in $\delta f_\nu$: substituting
$g_\nu\to g_\nu+\delta f_\nu$ in $\chi_\pm$ [Eq.~\eqref{eq:born}] and keeping the
first-order term gives the additive correction
\begin{equation}
  \delta\Gamma^{\rm SD}_{n\to p}=\Kw\!\int_0^\infty\! p^2\,\dd p\,
    \left[\delta\chi_+(E)+\delta\chi_+(-E)\right],
  \qquad
  \delta\chi_\pm(E)=(E_\nu^\mp)^2\,\delta f_\nu(E_\nu^\mp)\,g(-E),
\end{equation}
i.e.\ the Born kernel with the neutrino Fermi--Dirac replaced by its distortion
$\delta f_\nu$ evaluated at the same shifted neutrino energy $E_\nu^\mp=E\mp\Gap$
(and $\chi_+\to\chi_-$ for $p\to n$). primat supports two forms of
...
Eq.~A33)
\begin{equation}
  n_i = g_i\left(\frac{m_iT}{2\pi}\right)^{3/2}e^{\xi_i-x_i},
...
\begin{equation}
  n_i^{\rm NSE} = \frac{g_i m_i^{3/2}}{2^{A_i}}
    \left(\frac{n_p}{m_p^{3/2}}\right)^{Z_i}
    \left(\frac{n_n}{m_n^{3/2}}\right)^{A_i-Z_i}
    \left(\frac{2\pi}{T}\right)^{3(A_i-1)/2} e^{B_i/T},
...
\begin{equation}
  \frac{\gamma_{j\to i}}{\gamma_{i\to j}}
   = \frac{\prod_i \tfrac{1}{N_i!}\left[g_i(m_iT/2\pi)^{3/2}\right]^{N_i}}
          {\prod_j \tfrac{1}{N_j!}\left[g_j(m_jT/2\pi)^{3/2}\right]^{N_j}}\,
     \exp\!\left(\frac{\sum_j m_j-\sum_i m_i}{T}\right),
\end{equation}
...
\cite{AME2016}. primat computes $(\alpha,\beta,\gamma)$ once per reaction in
\code{compute\_detailed\_balance\_coefficients} and stores them in
\code{data/csv/detailed\_balance.csv}.

% =====================================================================
\section{Nuclear reaction list and sources}\label{app:reactions}
% =====================================================================
primat's networks are plain lists of reaction names under
\code{data/nuclear/networks/} (the \code{small}, \code{small\_parthenope} and
\code{large} files, or any user-supplied network of the same form); each name
maps to a rate table under \code{data/nuclear/tables/} and a source code in
\code{data/csv/reactions\_large.csv}'s \code{ref} column (also readable from
each table's own header line as \code{ref=...}).
...
split into the three tiers of \S\ref{sec:eras}: the \code{small} network, the
additional reactions that appear at \code{network="large", amax=8} (the exact
reaction set of the former ``medium'' network), and the remaining reactions
of the full \code{large} network. This appendix is regenerated directly from
that data by \code{manual/generate\_tab\_reactions.py}, so it is guaranteed to
match what \code{load\_network} actually loads for each of the three tiers.
Throughout, $a\equiv{}^4$He ($\alpha$), $d\equiv{}^2$H, $t\equiv{}^3$H,
$g\equiv\gamma$, $n,p$ the nucleons, and $e^\mp$ the leptons of $\beta$-decays
(one-way, shown with $\to$). A $^\dagger$ marks rates computed from an
analytic formula rather than a tabulated grid (hard-coded in PRIMAT-Main.m,
including every $\beta$-decay); ``(borrowed)'' marks a rate table copied
verbatim from another reaction's (e.g.\ a mirror/analogue reaction with no
...
\textbf{TALYS2} the TALYS statistical-model code (Koning et al.\ 2023);
...
\textbf{Nubase} NUBASE2020, Kondev et al.\ (2021, $\beta$-decay half-lives not
in Aud03) \cite{Nubase2020};
\textbf{Trezzi2017} Trezzi et al.\ (LUNA, 2017);
\textbf{Bar16} Barbagallo et al.\ (n\_TOF, 2016);
\textbf{Rij19} Rijal et al.\ (2019).
The remaining short author--year codes -- all carrying the $^\dagger$
...
\texttt{CF88\&MF89}, \texttt{Des99Bea01}) -- decode by the same
first-author/year rule, but their full bibliographic entries are not
reproduced here; they are catalogued in the compilations of Coc et al.\
\cite{Coc2012} and the NACRE/NACRE-II evaluations \cite{NACRE2} and are not
all individually retrievable from the shipped tables.
...
\code{nuclides.csv} (under \code{data/csv/}): the neutron and proton
numbers $(N,Z)$,
...
\code{manual/primat\_doc\_figures.ipynb}, which writes PDFs into
\code{manual/figures/}. To refresh them after changing the code, run the notebook
top to bottom, or non-interactively:
\begin{lstlisting}[style=sh]
jupyter nbconvert --to notebook --execute --inplace primat_doc_figures.ipynb
\end{lstlisting}
Similarly, App.~\ref{app:reactions}'s reaction-list table is regenerated (from
the current network/rate-table data, not from a run) by
\begin{lstlisting}[style=sh]
python manual/generate_tab_reactions.py
\end{lstlisting}
Then recompile the document. The notebook uses only the public primat API
(the Python backend, so that the intermediate background/weak-rate/reaction-rate
attributes of \S\ref{sec:usage} are available) and doubles as worked examples
for extracting the background history, weak rates, reaction rates and
abundance trajectories from a run.
...
\bibitem{Nubase2020}
  F.~G.~Kondev et al., \emph{The NUBASE2020 evaluation of nuclear physics
  properties}, Chin. Phys. C \textbf{45} (2021) 030001.
\bibitem{Cooke2018}
