...
D/H        = 2.4356967e-05
He3/H      = 1.0396057e-05
He3/He4    = 1.2676090e-04
Li7/H      = 5.502881e-10
Li6/Li7    = 1.418238e-05
...
directly; every key of \code{config.DEFAULT\_PARAMS} (73 keys) is accepted.
\begin{lstlisting}[style=py]
from primat import PRIMAT

result = PRIMAT({"Omegabh2": 0.022425, "network": "large", "amax": 8}).solve()
print(f"YP  (BBN) = {result['YPBBN']:.8f}")   # 0.24700624
print(f"D/H       = {result['DoH']:.7e}")      # 2.4350575e-05
...
backends are validated to agree to $\lesssim3\times10^{-9}$ on $\DoH$ for the
same inputs, up to one known $\sim1.7\times10^{-8}$ small-network gap awaiting
root-causing, see \code{tests/test\_backend\_parity.py}). Pass
...
reaction networks (\S\ref{sec:custom-networks}) and the full time-evolution
schema (below) are supported identically on both backends; incremental MC
sample reuse (\code{prev=}, \S\ref{sec:mc-usage}) and \code{extra\_rho}/
\code{background=}/\code{decay\_era} remain Python-only.
...
an \code{EvolutionResult} with the unified schema of
\code{primat.evolution}/\code{PRIMAT.md} \S7 -- \code{t\_s}, \code{a},
...
shipped \code{primat/data/} tree -- must contain \code{NEVO/}, \code{weak/},
\code{plasma/}, \code{nuclear/} and \code{csv/}) and \code{user\_nuclear\_dir}
...
``Nuclear reactions'' sidebar group, two popups -- \textbf{Import custom
network} and \textbf{Create custom network} -- that let a user start from any
named network, toggle reactions in/out grouped by mass-number category,
substitute or upload an alternate rate table per reaction, add brand-new
reactions, then either save the result as a re-importable \code{.zip} or run
BBN with it directly (\S\ref{sec:custom-networks}); a results tab exposes the
