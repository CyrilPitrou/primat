...
Both are generated from the same three sources of truth: ``DEFAULT_PARAMS``
...
Usage::
    ...
    python -m primat.tools.gen_param_templates          # write both files
...
_TEMPLATE_DESCRIPTIONS = {
    ...
    "numba_installed": "re-checked at runtime; enables JIT kernels if available (unused on the C backend, kept for round-trip parity)",
    ...
    "weak_rate_cache": "if False, never load the weak-rate cache (always recompute)",
    ...
    "rescale_nuclear_rates": "vary all nuclear rates by a uniform factor (sensitivity studies)",
...
def check() -> bool:
    """Compare the committed template files against freshly generated text.

    Returns True (and prints nothing) if both match; otherwise prints which
    file(s) are stale and returns False. Used by both the ``--check`` CLI
    flag and ``tests/test_docs_consistency.py``.
    """
    ok = True
    for path, generate in ((_TEMPLATE_PY, generate_run_explanatory),
