...
- **A parameter reference on the documentation site.** `docs/parameters.md`
  lists every one of primat's run parameters with its default and a one-line
  explanation, grouped as `primat/config.py` groups them. It is generated by
  `primat.tools.gen_param_templates` alongside the two copy-and-edit
  templates, so it cannot drift from the code. Until now the site's only
  answer to "what can I set?" was to read the source.
- **A first tutorial.** `docs/tutorials/first-run.md` walks from a fresh
  install to a result the reader understands: run the standard case, read
  each number it prints, change the baryon density and see what moves, then
  look inside the run. The notebook section it precedes is now called
  "Worked examples", which is what it always was.
- **A page for the graphical interface.** `docs/howto/gui.md` describes what
  `primat-gui` offers — the parameter form, the four result tabs, the quick
  Monte Carlo, and the two archives that reproduce a session elsewhere. The
  GUI was one of the four advertised ways to use primat and had no page.
- **`cache_dir` is documented.** The parameter that redirects primat's only
  writable data — needed on any read-only install — now has a section in
  `docs/howto/data-overlays.md` rather than a cross-reference to a page that
  did not mention it.
- **API pages for `primat.background`, `primat.network_data` and
  `primat.neutrino_history`**, the three modules the how-to guides tell
  readers to consult. Every API page now opens by saying what its module is
  for and which how-to covers the same ground.
...
### Changed
- **Three run parameters changed name or went away, and the old spellings have
  no fallback.** `numba_installed` is now `use_numba` and `atol_large_LT` is
  now `atol_LT`. Neither old name described what it set: the first reads as
  something primat detected when it is a knob you turn (and turning it off is
  not numerically neutral — the plain-Python and compiled kernels differ in the
  last digits of the rate integrands), and the second was named after the
  `large` network while applying to the low-temperature era of every network.
  An old spelling is now an unknown key: a warning normally, an error under
  `strict_params`, with the usual "did you mean …?" suggestion. A saved
  parameter dictionary or `.ini` file naming either one needs editing.
- **The graphical interface's network builder no longer loads every rate table
  on every click.** Each reaction row kept its rate table in a pop-over, and
  the interface rebuilds a pop-over's contents whether or not it is open — so
  opening the builder and toggling a single reaction put 16.85 MB of table text
  into the page for the `large` network, on every interaction. A table is now
  read only when you press its "Show rate table" button, and opens full width
  below its row.
- **The graphical interface says what its buttons do.** Three on-screen strings
  sent the user to a button that had been removed; the same network reported
  two different reaction counts one screen apart; the evolution plot's y-axis
  called a per-baryon abundance a mass fraction; the download buttons named a
  filename instead of its contents; and one parameter carried three different
  labels. Seventeen strings in all.
- **Every shipped notebook runs, and shows what it claims to show.** Seven of
  the ten carried no figures; the uncertainty-budget notebook varied no nuclear
  rate at all, which is the one thing it promises; eight crashed on Colab,
  which the README advertises; `make bench` had been benchmarking a usage
  message; and the C backend's one documented command did not run. All ten are
  re-executed with their figures, and `primat-c/examples/` gained a
  `run_small.ini` alongside the two it had.
...
### Removed
- **`rescale_nuclear_rates`.** It was accepted and stored, and read by nothing
  on either backend, so setting it did exactly what leaving it alone did. Vary
  a reaction rate with `p_<reaction>` (log-normal, in units of the table's own
  uncertainty) or `delta_<reaction>` (a direct fractional shift); each applies
  on its own. primat now has 95 run parameters rather than 96.
...
- **A nonsensical proton or neutron mass no longer produces a confident wrong
  ...
  rate came back as "not a number": the pure-Python backend reported
  `YP = 0.98` and `D/H = 0.387` as though they were results and wrote that
  table to the cache, and the compiled backend read past the end of its own
  table, taking the calling Python session down with it. Both backends now
  check the table before writing it and again after loading one, and reject a
  table with fewer than two usable points.
...
- **A `GN` override reaches the baryon density on both backends.** Newton's
  constant enters the baryon-to-photon ratio, which the compiled backend was
  not recomputing when `GN` changed — so the same override moved D/H by
  +0.98 % there against +2.6 % in pure Python, against a 5e-5 agreement
  budget. Restating `Omegabh2` at its own default value *after* `GN` in a
  parameter dictionary silently fixed it, so the answer depended on the order
  of the keys.
- **Eight settings that were accepted and silently wrong are now refused or
  flagged.** At least four electron-table points (the interpolant's minimum)
  and `DeltaNeff >= -3` (below it the neutrino energy density is negative) are
  now errors. A sampling grid below 20 points per decade, a master rate grid
  below 50 points, `amax < 4` (which reports `YPBBN = 0` because helium-4 is
  not in the network, not because none is made), a neutrino chemical potential
  combined with incomplete decoupling, and a decay era on a network other than
  `large` all warn, naming what they cost.
- **The published time evolution no longer says neutrinos are hotter than
  photons.** It did, on 82 of its 500 rows. The photon temperature was
  interpolated in log-log and the three neutrino temperatures linearly, and
  above 6 MeV — where the two are physically equal to better than 1e-09 — the
  two schemes drifted apart by up to 5.5e-06. Both backends now interpolate the
  dimensionless ratio and multiply by the photon temperature, so the bound
  holds by construction. This reached you through `output_time_evolution`, the
  background table's Hubble column and the interface's plot.
- **`T(a)` stays a temperature past the end of its own table.** Both backends
  continued it with a straight line, which reaches zero at about twice the last
  tabulated scale factor and goes negative beyond it. It now continues the
  radiation-domination power law, `T ∝ 1/a`. Every value inside the tabulated
  range is unchanged in every digit.
- **The Windows console guard asks about the stream it is protecting.** The
  check that swaps the box-drawing banner for plain characters read standard
  output on both backends, while the banner is written to standard error — so a
  run with results redirected to a file and the console attached to errors
  still raised the encoding error the guard exists to prevent, and, the other
  way round, chose plain characters for a file that would have taken the
  originals.
...
- **`weak_rate_cache=False` never forced the expensive recompute, and the
  ...
  non-thermal rate is gated by that flag. `docs/howto/weak-rate-cache.md` now
  says so, and says how to redo the thermal integration when you mean to.
- **The result-dict page was missing four keys.** `He3oHe4`, `Li6oLi7`,
  `YCNO` and `Y_final` — the last being the only route to any nuclide that is
  not one of the six headline observables. The Monte-Carlo guide also
  promised that `Li6oLi7` and `YCNO` are *always* present; both are
  network-conditional, and neither exists on the default `small` network.
- **The install page no longer claims the two backends give identical
  results.** They agree to about 7e-06 relative on D/H, which is what
  `docs/api/backend.md` now states, with the measured numbers.
- **The `extra_rho` example runs.** The only worked example of primat's
  generic energy-density plug-in referred to an undefined variable, so it
  raised `NameError` mid-solve as written.
- **Stale timings.** The performance page's Python-backend numbers were 15 %
  low and its C-versus-Python ratio 12x where it is now 10x; the n<->p
  recompute cost, quoted as "~1.8 s" in three places, is 0.4 s on the C
  backend and 8 s on the pure-Python one.
...
- **Running the test suite no longer changes the shipped weak-rate tables.**
  One test recomputed the n↔p rates and saved them over the tracked copies,
  which is enough to move the repository's own validation reference
  (YP 0.24700086 → 0.24700060) while every pinned tolerance still passes. A
  clone that ran `pytest` therefore stopped matching the numbers it
  documents.
...
- **The NEVO heating function is clamped to N ≥ 0** (both backends). Heating is
  ...
  Together with the interpolant fix above, this moves the default run by
  ~4e-6 relative in D/H and Li7/H — two orders of magnitude inside the ±3e-9
  D/H regression tolerance, with cross-backend agreement unchanged.
...
- **n↔p weak-rate cache keys** (`WEAK_RATE_FORMAT_VERSION` 1 → 4, both
  ...
  With a rebuilt extension both backends reproduce the previous D/H, YP and
  Neff in every digit.
