...
```bash
pytest tests/                          # everything: 756 tests, ~12 min
...
Timings measured 2026-08-27 on the C backend; the pure-Python fallback is
slower on every tier that solves. The CI lane is most of the suite: the `slow`
marker excludes 351 tests, but 298 of them come back with `or solve`.
...
| Promise | Enforced by |
|---------|-------------|
| The two backends compute the same physics and return the same shapes. | `test_backend_parity.py`, and "Known cross-backend divergences" below for the three places they knowingly differ |
