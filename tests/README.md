...
pytest tests/ -m reference             # only the tight high-precision regression, ~10 s
...
A second, easier-to-miss skip: the tests that drive the **standalone**
`primat-c` executable need it to exist, and `make test` in `primat-c/` builds
only the unit-test programs. Run plain `make -C primat-c` as well, or those
tests skip without a word at `-q`.
...
Start here if the question is "what does primat promise, and what would catch
it breaking?". Each row names the file to open first, and where a promise is
split across files, what each one covers. The Structure table below then says
what every file checks in detail.

| Promise | Enforced by |
|---------|-------------|
| The two backends compute the same physics and return the same shapes. | `test_backend_parity.py`, and "Known cross-backend divergences" below for the four places they knowingly differ |
| The two backends read and write the *same* cache files, safely. | `test_cache_parity.py` |
| A cache goes stale when — and only when — something it read changes. | `test_cache_constant_deps.py` (which constants each cache reads), `test_off_default_params.py` (values that must *not* rename a cache file), `test_cache_utils.py` (the hashing machinery itself) |
| A damaged or half-written cache is never trusted. | `test_state_and_cache_robustness.py` |
| Nucleons and charge are conserved, and abundances stay positive. | `test_invariants.py` (throughout every era), `test_network_builder.py` (formally, per reaction), `test_large_network.py` (end to end on 429 reactions) |
| `solve()` returns the documented result dict. | `test_api.py` (the keys and their types), `test_backend_parity.py` (both backends return the same set), `test_docs_consistency.py` (the table in `README.md` lists them) |
| The time-evolution TSV has one schema, whoever wrote it. | `test_evolution.py` (the Python writer + round trip), `test_backend_parity.py` (the C writer's header, byte for byte) |
| A bad rate table is refused rather than integrated. | `test_rate_table_domain.py` (from disk), `test_gui_robustness.py` (from an upload or a zip) |
| The published abundances are reproducible. | `test_regression.py` against "Validation reference" below, `test_runfiles.py` for the documented `primat_run.py` check |
| The two backends narrate a run identically. | `test_verbose_parity.py` |
| The shipped data tree is exactly what its generator produces. | `test_network_generation.py` |
| The documentation's numbers are the code's numbers. | `test_docs_consistency.py` |

## The C backend's own suite

This file covers the Python suite only. `primat-c/` carries a second one —
27 unit files plus a fuzz tier under `primat-c/tests/` — which is where the
C-only guarantees live (parsers, memory safety, the standalone CLI). Run it
with `make test` from `primat-c/`; `make debug-test` repeats it under
AddressSanitizer. The Python suite reaches the C code only through the
compiled extension, so a C-side regression with no Python-visible effect
fails there and nowhere here.
...
| `test_weak_rates.py` | n↔p weak rates: Fermi-Dirac helpers, the Fermi-Coulomb correction, the neutron-decay phase-space integral `ComputeFn`, that the two loaded rate interpolants (forward/backward) are positive and obey detailed balance (ratio → 1 at high T); (`slow`-tier) that `RecomputeWeakRates`'s recompute path (`weak_rate_cache=False`) reproduces the shipped, git-tracked cache to 1e-6 relative — the only pin on the *contents* of the n↔p rate tables, and so on the individual correction terms; and that `_setup_fd_impls` re-wraps the module-level `FD_*` implementations (jitted vs pure-Python) every time `use_numba` changes, rather than latching on the first call. Plus the vegas-less fallback's honesty: that `_dblquad_checked` reads quadpack's error estimate instead of dropping it, and that the vegas-not-found warning says what the fallback costs (hours, not minutes) and names both ways out. |
...
| `test_refactor_invariants.py` | Properties the performance refactor relies on: MC results independent of `n_jobs`, `eta0b` recomputed on reassignment, GN/tau_n overridable, electron-thermo tabulation ≈ exact integrals, and the two bit-exact speedups on the solver's hot path: the background's scalar interpolant fast path against scipy, and BDF's direct-LAPACK dense LU against plain `method="BDF"`. |
...
| `test_invariants.py` | What the physics *requires* of any correct answer, as opposed to what this code produced: baryon number and positivity at every accepted step of every era (and over the published time evolution, on both backends, and through the decay era); the one-way direction of time, temperature and the scale factor; `T_nu ≤ T_gamma` for all three flavours; comoving entropy conserved when nothing heats the neutrinos; and the analytic limits — `Neff = 3` under instantaneous decoupling with no QED, `Gamma(n→p) → 1/tau_n` as `T → 0`, detailed balance between the two n↔p rates above decoupling (including the finite-mass recoil factor), Saha equilibrium nulling every thermonuclear reaction's net flux (which cross-checks `detailed_balance.csv` against `nuclides.csv`), the `D/H ~ (Omega_b h²)^-1.6` power law, the Li7 valley's *position*, the leftover free neutron decaying as `exp(-Δt/tau_n)`, and that `T(a)` stays a positive temperature past the end of its own node range. Every bound is a physical statement with slack, not a measurement to be updated when a number moves. |
...
| `test_regression.py` | Final abundances: loose default-precision sanity checks, tight `reference`-marked checks against the "Validation reference" values below, the per-nuclide table check (every cell of it, at 1e-4 relative), the physical-effect guards for the n↔p correction flags (Born mode and CCRTh both lowering YP), no-numba full solve checks (pure-Python kernels must match JIT to 1e-4), the `amax` cutoff verification (the large network filtered to A ≤ 20 matches the full large network's light elements to ~1e-3), and what the master rate grid's default resolution costs — refining `rate_grid_npts` 4× still moves `Li7/H` by ~1e-4 and `D/H` by ~6e-6 with the ODE tolerance already converged, which is why `Li7/H`'s last two reported decimals are grid artefacts (`docs/performance.md`). |
...
| `test_docs_consistency.py` | Every documentation claim that no other test would catch, none of them a solve, so the file stays in the fast lane: the `CPRIMAT_VERSION` ↔ `pyproject.toml` version sync; the Streamlit-Cloud chain (`requirements.txt` → an existing `wheels/*.whl` at the right version); that both generated parameter templates are byte-for-byte what `primat.tools.gen_param_templates` produces *and* list exactly the `DEFAULT_PARAMS` keys; that `runfiles/primat_reference_run.py` still declares the four reference-run parameters verbatim and `PRIMATConfig` recognises every key it sets; `PRIMATConfig`'s `save_nTOp`/`save_nTOp_thermal` defaults; a batch of README guards (the `--set KEY=VALUE` form, the MC key names, the Python-only-features list vs `backend.py`'s actual gate, the unified evolution-schema columns, the result-dict key table, the per-reaction rate columns); that the quick-start YP and D/H quoted by **both** landing pages, `README.md` and `docs/index.md`, are exactly the tracked reference constants; that `notebooks/README.md`, `docs/tutorials/index.md` and this file each list every file they index; and that this README's "Validation reference" tables — observables *and* the per-nuclide table — quote exactly the constants in `tests/reference_values.py`; that every human-readable citation of the methods paper gives CITATION.cff's own volume and pages (and that no document anywhere prints the DOI suffix as a volume); that the `<reaction>_frwrd` column counts README and `primat-c/include/nuclear_network.h` quote are the real `n_reac - 1` per network; and that this file's own two test totals are what pytest collects. |
| `test_gui.py` | The optional Streamlit GUI (`primat.gui`): `import primat` does not pull in `primat.gui`/streamlit; the parameter-form metadata covers `amax` (the one `None`-default key) and the network choices; an end-to-end `AppTest` run of `primat/gui/app.py` reproduces `test_cli.py`'s pinned default-run values (Neff/YPBBN/D-H and the per-nuclide table) -- i.e. the GUI drives `PRIMAT` identically to the CLI; the abundance-evolution panel renders with its default "light elements" nuclide selection; the `amax` widget appears for every network; every curated widget and every "Output tables" download carries a help text, and the "Physics" group's sub-headings run Weak rates -> Plasma physics -> Nuclear QED; and an invalid flag combination (`spectral_distortions=True` with `incomplete_decoupling=False`) is shown as a clean `st.error` rather than a traceback. Skipped entirely if the `gui` extra is not installed. |
| `test_backend_parity.py` | Backend parity: `primat._primat_c` (C) vs `primat.main.PRIMAT` (Python) — the result-dict shape, the written evolution-TSV header byte for byte, and numerical agreement, both at the default precision and again with both backends' ODEs converged, where controller noise is gone and only the structural divergences listed below survive. Its module docstring carries the current measured gap per network and attributes it term by term. |
| `test_cache_parity.py` | Cross-backend **cache** parity, the companion to `test_backend_parity.py`: the two backends *share* every on-disk cache, and this is what makes sharing safe rather than merely convenient. Drives both backends into separate `cache_dir`s on a coarse grid and asserts (a) **hash identity** — both emit the same `nTOp_<hash>.txt` / `electron_thermo_<hash>.txt` filename and the same QED fingerprint header, which pins `cpr_weak_rate_fingerprint`/`cpr_constants_hash`/`cpr_qed_fingerprint` against their Python counterparts field-for-field, per cache (each cache hashes only the constants it reads, so equal hashes prove the two sides declare the same subsets) — (including that a 1 % shift in `me` re-keys both backends' caches *identically*, so an override can neither reuse the default-constant table nor split the shared cache) — and (b) **column agreement** at documented, measured tolerances (n↔p rates 1e-8; electron-thermo 1e-9 above 1e-15 of each column's peak, with a column-scale check covering the tail below it, where one backend can underflow to exact zero against the other's ~1e-17; QED e2/e3 1e-15 on a column-scale metric, since pointwise-relative is dominated by the Boltzmann-suppressed tail). The CCRTh thermal table's *contents* are deliberately excluded (independent Monte-Carlo streams, minutes to recompute); its fingerprint is still pinned via a both-backends-hit-the-shipped-file assertion. Skipped without the C extension. ~8 s. |
| `test_background.py` | `StandardBackground` on its own — a `cfg` + `Plasma`, no nuclear network, so each check costs a few hundred ms instead of a full solve. Pins the two documented properties of `external_scale_factor` that no full-solve test reaches: reading `a(T)` straight from the NEVO table's `x` column agrees with the entropy-conservation ODE to ~1e-5 in both `a(T)` and `t(T)`, and outside the table's temperature range both modes extrapolate as radiation domination requires (`a ~ 1/T`, `t ~ 1/T²`). |
...
| `test_decay_rates.py` | Radioactive decay in the `large` network, in two parts. First, that `decay_reverse_rates=True` is numerically inert: every β-decay releases energy, so its detailed-balance γ is negative and the reverse rate is suppressed by `exp(γ/T_nucl)` with `γ/T_nucl < -100` for *every* decay — checked as a zero `bwd_cap`, with no solve needed. Second, the decay (DT) era that runs after `T_end`: known half-lives (Na22, C14) decay away by 1 Gyr while stable nuclides do not, the decay matrix conserves baryons, and `expm_multiply` agrees with Radau over the first 10 yr. |
...
| `test_deuterium_network.py` | `network="large", amax=2` reproduces the standalone `deuterium` network the `amax` filter replaced — the clean-slate starting point for a custom network. The A ≤ 2 slice keeps two thermonuclear reactions rather than one (`p_p_n__d_p` shares D's product channel), which leaves the physics unchanged: D/H matches the deuterium-only value to ~1e-9, baryon number is conserved, and every nuclide beyond D stays at zero. |
...
| `test_gui_custom_network.py` | The GUI's "Manage networks" dialog and the "Create custom network" dialog it gates (`primat.gui.params_form`), driven end to end through Streamlit's `AppTest` — listing/removing/renaming, loading from a zip, toggling reactions, switching the base network, editing a decay rate, adding a reaction, applying the result. Every test is a regression guard for a specific way the popup rendered or solved the wrong network; the module docstring names them one by one, along with the `AppTest` quirk that dictates how each test opens and closes the dialog. |
...
| `test_gui_run_view.py` | `GuiRun`, the backend-agnostic stand-in the GUI hands to its panels in place of a solved `PRIMAT`, must be indistinguishable from the real thing: same `abundance_names` ordering, same `get_quantity`, same `__getitem__`/`T_of_t` interpolators. A mismatch renders subtly wrong data without raising, which is why this compares the two directly instead of through `AppTest` — and why it needs neither streamlit nor plotly to run. |
| `test_neutrino_history.py` | `make_neutrino_history`'s factory dispatch: which `NeutrinoHistory` a given `PRIMATConfig` assembles (temperatures, heating, spectral distortion, extra energy density) and that each one satisfies the protocol its callers rely on. Builds the neutrino sector on top of an initialised plasma only — no BBN solve. |
| `test_notebooks.py` | Notebook smoke test: papermill-executes **every notebook in the repository** — the ten under `notebooks/` (Monte-Carlo ones with their sample count cut to 3 through papermill's `parameters` cell), plus the two that live elsewhere and derive the repo root from their own working directory, so each runs from a throwaway copy of its own folder: `generate_rates/thermal_average.ipynb` and `manual/primat_doc_figures.ipynb`. Catches API drift that renames something a notebook reads. |
| `test_nuclear.py` | The auto-derivation fallback in `reaction_stoichiometry` and the duplicate-entry check in `load_network`. |
| `test_verbose_parity.py` | The two backends must narrate a run identically: a verbose run of each CLI is compared line for line after normalising the `-py`/`-c` tag suffix, the backend each reports for itself, and wall-clock timings. Adding a stage to one backend and not the other is a divergence like any other; this is the rule that had no test until then, and the two streams had already drifted in two places by the time it was written. Genuinely backend-specific lines (the numba report) are excluded by content, with a reason. |
...
| `test_rate_variations.py` | The two knobs that move a single reaction rate: `p_<reaction>` (shift by `exp(p × σ)` off the median, the MCMC sampling handle) and `delta_<reaction>` (a direct fractional shift stacked on top of it). Checks that each actually moves the abundances, that returning both to zero reproduces the baseline to floating-point precision, that `delta_*` works when passed through `run_bbn`'s params dict, and that the MC runner turns rate uncertainty into non-zero spread in the observables. |
...
| `test_sensitivity.py` | `primat.sensitivity`'s public contract, the one `notebooks/Sensitivity.ipynb` is written against: `sensitivity_table` returns a `SensitivityTable` of the right shape sharing one central solve; each of the three target flavours (multiplicative parameter, nuclear-rate `delta` rescaling, additive knob) reproduces the exact symmetric finite difference a hand-written loop would give; and the rendering helpers and input validation behave as documented. |
| `test_state_and_cache_robustness.py` | Behaviour when many runs share one process, and when a cache file is damaged: a truncated electron-thermo cache is recomputed rather than trusted (and must not abort the process), a truncated n↔p cache is reported with the same sentence on both backends, a cache that could not be written completely is never installed, coexisting configurations reproduce their solo results, the process-wide numba rebinding survives concurrent flipping, and the two process-global caches evict rather than grow. |
...
other's physics and numerics. Four places knowingly do not, and this is the
...
| **HT-era integrator.** Python integrates the n↔p-only HT era with `LSODA`, C with Dormand-Prince RK45. | **Intentional, and now a negligible term.** It once measured as the whole YP gap; re-measured after the `t(T)` coordinate fix above it accounts for ~1e-10 of it. Aligning both on BDF was tried and *degraded* YP parity, so the two methods stay as they are. |
